/*
take order id in rabbitmq and push redis order id status: delivering
*/
package main
...
func main() {
	...
	for msg := range msgs {
		...
		go func(orderid string) {
			...
			err := client.Set(ctx, orderid, "delivering", 0).Err()
			if err != nil {
				fmt.Printf("Error setting initial order status for order %s: %s\n", orderid, err)
				return
			}
			//status wait 30 second in redis then start delivered status
			...
			//marshall orderid and send created queue
